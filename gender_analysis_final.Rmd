...
modelrm2 <- function(parent,food){
  sprintf("
         ...
         # direct effect, the effect of sex on food intake
         Q6_11 ~ c(cg1,cg2)*%s + iEO + RaceBlack + RaceAsian + RaceHispanic + RaceOther + Sex_Girl + Education_2 + Education_3 + Education_4 + FoodSecure
...
summary(checkMonFVmod, fit.measures=T, standardized=T, ci=TRUE)
```
...
modelrm <- function(parent,food){
  sprintf("
         ...
         # direct effect, the effect of sex on food intake
         Q6_11 ~ c*%s + iEO + RaceBlack + RaceAsian + RaceHispanic + RaceOther + Sex_Girl + Education_2 + Education_3 + Education_4 + FoodSecure
...
```{r}
checkMonFVmod2 <-sem(modelrm("MONITORING","FV_freq"), data = scaled_data, estimator = "MLM")
```

```{r}
checkMonSSBmod_boot <- bootstrapLavaan(checkMonSSBmod2, R = 5000, FUN = fct, iseed = 8970)
checkMonFVmod_boot <- bootstrapLavaan(checkMonFVmod2, R = 5000, FUN = fct, iseed = 8970)
```
...
```{r}
#save.image(file = "/Users/gracer/Library/CloudStorage/OneDrive-UniversityofWyoming/0. Lab/M2AENAD Lab - Documents/RESEARCH/multi-state/gender_paper/gender_analysis121423.RData")
```
